...
// 사용자가 입력한 값을 받아 올 때 = request.getParameter():String getParameter 요청객체 String 타입
// 사용자가 입력해야 하는 값이 몇개 인가? - 4개 - request.getParameter("컬럼명") 4번
...
// LoginServer에서 직접 MemberDao클래스의 메소드를 호출하는것이 아니라
// 중간에 MemberService를 경유하는 코드로 구성이 되어있다.
// XXXServlet - XXXService - XXXDao(MyBatis, Hibernate 유연한 구조)
// 인스턴스화 - 생성자 호출 - 싱글톤(getInstance?) - 객체 주입 (DI, IoC)
// 인스턴스화 시점이나 위치에 따라 차이가 있다 - NullPointerException
// 이른 객체 주입 또는 게으른 객체 주입 -> 퍼포먼스
...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // 다시 백엔드에서 프론트 엔드로 내보내는 값(결과물) - 사용자가 입력한 값을 받고 그 결과물을 백엔드를통해 프론트로 보내는
